...
class GraphBlock(nn.Module):
    def __init__(self, node, d_model , conv_channel, skip_channel,
                        gcn_depth , dropout, propalpha ,seq_len, node_dim):
        super(GraphBlock, self).__init__()

        self.nodevec1 = nn.Parameter(torch.randn(node, node_dim), requires_grad=True)
        self.nodevec2 = nn.Parameter(torch.randn(node, node_dim), requires_grad=True)
        # self.nodevec2 = nn.Parameter(torch.randn(node_dim, node), requires_grad=True)
        self.start_conv = nn.Conv2d(1 , conv_channel, (d_model - node + 1, 1))
    ...
    def forward(self, x):
        nodevec1 = torch.tanh(self.lin1(self.nodevec1))  # equation 1
        nodevec2 = torch.tanh(self.lin2(self.nodevec2))  # equation 2
        a = torch.mm(nodevec1, nodevec2.transpose(1, 0)) - torch.mm(nodevec2, nodevec1.transpose(1, 0))  # equation3
        adp = F.relu(torch.tanh(a))  # equation 3

        # adp = F.softmax(F.relu(torch.mm(self.nodevec1, self.nodevec2)), dim=1)  # ADP [node, node] torch.mm表示矩阵相乘

        # adp_visual = adp.detach().cpu().numpy()
        # # p = sns.heatmap(adp_visual, vmin=adp_visual.min() - 0.01, vmax=adp_visual.max() + 0.01, cmap='viridis_r', shading='gouraud') # 彩虹热力图
        # # p = sns.heatmap(adp_visual[:31, :31], vmin=adp_visual.min() - 0.01, vmax=adp_visual.max() + 0.01, cmap='viridis_r') #普通热力图
        # p = sns.heatmap(adp_visual[:31, :31], vmin=adp_visual.min() - 0.01, vmax=adp_visual.max() + 0.01)
        # s = p.get_figure()
        # s.savefig(
        #     './figures/adp.png',
        #     dpi=600,
        ...
        # 对输入进行一次 1x1 的卷积操作,将输入投影到潜在空间，得到初始特征 x
        out = self.start_conv(out)
        out = self.gelu(self.gconv1(out, adp))
        # out = self.gconv1(out, adp) + self.gconv1(out, adp.transpose(1, 0))
...
class mixprop(nn.Module):
    ...
    def forward(self, x, adj):
        # 对输入的邻接矩阵 adj 添加单位矩阵,以确保每个节点都有自连接
        ...
        for i in range(self.gdep):
            ...
            out.append(h)
        # 将 out 列表中的张量在通道维度上拼接起来,形成输出张量 ho
